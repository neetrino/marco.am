...
| ---- | -------------------------------- | --------------- |
| 1    | Infra & API կոնտրակտ             | `62%`           |
| 2    | Գլխավոր էջ (Home) — տվյալներ     | `23%`           |
...
**Ընդհանուր նախագծի առաջընթաց (backend).** `50%` — *(12 փուլերի միջին տոկոս, մոտավոր)*։
...
| --- | --------------------------------------------------------------------------------------------------------- | ---------- | ---------- |
| 1.1 | Հաստատել API կոնտրակտը frontend-ի հետ — REST կամ GraphQL, auth headers, սխալների մոդել                    | 80         | 🔄         |
| 1.2 | Միջավայրեր (dev / staging / prod) — env, build/deploy-ին համապատասխան կոնֆիգ (ըստ թիմի պատասխանատվության) | 45         | 🔄         |


*Նշումներ.* REST JSON `src/app/api` (ներառյալ `v1`), JWT auth, RFC7807-անման `type`/`title`/`status`/`detail` — կան; GraphQL/OpenAPI փաստաթուղթ repo-ում որպես պաշտոնական կոնտրակտ չի երևում։
...
| 2.8 | Reels section (home) — կարճ ցուցակ / նախադիտում կամ deep link դեպի Reels էջ (տես Փուլ 11)                              | 15         | ⬜          |
