...
**5.4 ✅ ավարտված (2026-04-17), թարմացված (2026-04-29).** Վճարման եղանակ checkout-ում՝ միայն `**cash**` (կանխիկ առաքման ժամանակ) — `CheckoutForm` radio, `paymentMethod` դաշտը `POST /api/v1/orders/checkout` մարմնում։ Սերվեր՝ `resolveCheckoutPaymentMethod` / `normalizeCheckoutPaymentMethod` (`src/lib/constants/checkout-payment-method.ts`) — `**cash_on_delivery**` / `**cod**` → `cash`, դատարկ/բացակայող → `cash`; առցանց մեթոդների հին արժեքները (`arca`, `idram`, `card`, `visa` և այլն) մերժվում են։ `Payment` գրառումը պահում է `provider`/`method` = `cash`; `nextAction`՝ `view_order`։ Vitest՝ `checkout-payment-method.test.ts`։
