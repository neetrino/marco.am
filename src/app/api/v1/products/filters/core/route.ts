...
import { getProductsFiltersCoreCached } from '@/lib/cache/products-filters-redis';

export async function GET(req: NextRequest) {
  const startedAt = Date.now();
  return withApiRouteMetrics(
    '/api/v1/products/filters/core',
    'GET',
    async () => {
      try {
        const filters = parseProductsFiltersRequest(req);
        const result = await getProductsFiltersCoreCached({
        ...
        logger.error('Products filters core API error', { error });
