...
import { useMessagesAdmin } from './useMessagesAdmin';
...
export default function MessagesPage() {
  ...
  const { messages, loading, page, setPage, meta, fetchMessages } = useMessagesAdmin();
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !(await showPopupConfirm(
        t('admin.messages.deleteConfirm').replace('{count}', selectedIds.size.toString()),
      ))
    ) {
      return;
    }
        ...
        {loading ? (
          ...
          <>
            <div className="border-b border-slate-200 bg-slate-50/60 px-6 py-4">
              ...
              >
                <div
                  className={`text-sm font-medium ${selectedIds.size > 0 ? 'text-amber-900' : 'text-slate-600'}`}
                >
                  {t('admin.messages.selectedMessages').replace('{count}', selectedIds.size.toString())}
                </div>
                <Button
