...
export function CatalogBrandSection({
  ...
  const mode: CatalogMode = useNewBrand ? 'new' : 'existing';
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
      ...
      <div className="space-y-4 px-4 py-4 sm:px-5 sm:py-5">
        <SegmentedControl
          value={mode}
          options={[
            { id: 'existing', label: t('admin.products.add.selectExistingBrands') },
            { id: 'new', label: t('admin.products.add.addNewBrand') },
          ]}
          onChange={(next) => {
            onUseNewBrandChange(next === 'new');
            if (next === 'existing') {
              onNewBrandNameChange('');
            }
          }}
          ariaLabel={t('admin.products.add.brands')}
        />
