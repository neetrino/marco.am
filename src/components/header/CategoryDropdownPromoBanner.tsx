...
export function CategoryDropdownPromoBanner({
  ...
  const safeImage = toSafeImgAttributeSrc(imageUrl ?? null);
  ...
  return (
    <div className="relative mb-6 w-full overflow-hidden rounded-[24px] bg-gradient-to-b from-[rgba(255,202,3,0.45)] to-[#fff9e5] p-5 pb-6 md:mb-5 md:p-6">
      {safeImage ? (
        <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-[42%] sm:block" aria-hidden>
          <Image
            src={toDomSafeImgSrcString(safeImage)}
            alt=""
            fill
            sizes="280px"
            className="object-contain object-right-bottom opacity-90"
            unoptimized={shouldBypassNextImageOptimizer(safeImage)}
          />
        </div>
      ) : null}
      <div className={`relative ${safeImage ? 'sm:max-w-[58%]' : ''}`}>
        <span
        ...
        >
          <span className="min-w-0 whitespace-nowrap !text-[var(--marco-slate)] dark:!text-[var(--marco-slate)]">
            {ctaLabel}
          </span>
          <span
